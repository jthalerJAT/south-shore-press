...
import {
  requestClassifiedUploadUrl,
  createClassifiedAction,
  deleteClassifiedAction,
} from './actions';

const BUCKET = 'classifieds';
...
export function ClassifiedsAdmin({ classifieds }: { classifieds: ClassifiedRow[] }) {
  ...
  async function handleAdd() {
    ...
    setUploading(true);
    try {
      const signed = await requestClassifiedUploadUrl();
      ...
      const saved = await createClassifiedAction({
        year,
        month,
        day,
      ...
      if (!saved.ok) {
        setError(saved.error ?? 'Could not save the classified.');
        return;
      }
      setShowAdd(false);
      setFile(null);
  ...
  async function handleDelete(id: string, label: string) {
    if (!confirm(`Delete the classified dated ${label}? This can't be undone.`)) return;
    const res = await deleteClassifiedAction(id);
    if (!res.ok) {
    ...
    <div>
      <div className="flex items-center justify-between gap-4">
        <p className="text-sm text-zinc-600">
          Upload a classified-pages PDF. It appears here and in the Newspaper
          Creator&apos;s Classifieds tab, ready to drop onto a classifieds page.
        </p>
        <button
          type="button"
          onClick={() => {
        ...
        >
          {showAdd ? '× Close' : '+ Add New Classified File'}
        </button>
      </div>
      ...
      {showAdd ? (
        <div className="mt-5 rounded-lg border border-zinc-200 bg-zinc-50 p-5">
          ...
          >
            {uploading ? 'Uploading…' : 'Save Classified File'}
          </button>
        </div>
      ) : null}
            ...
            {classifieds.length === 0 ? (
              ...
              classifieds.map((c) => (
                <tr key={c.id} className="border-b border-zinc-100">
                  <td className="py-3 pr-4 text-zinc-900 whitespace-nowrap">{c.dateLabel}</td>
                  <td className="py-3 pr-4">
                    ...
                    >
                      {c.dateLabel.toUpperCase()} CLASSIFIED PDF
                    </a>
                  </td>
                  <td className="py-3 text-right">
                    <button
                      type="button"
                      onClick={() => handleDelete(c.id, c.dateLabel)}
